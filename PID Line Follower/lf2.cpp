...
const int sensorThresholds[8] = {750,750,750,750,750,750,750,750}; // Threshold to distinguish black and white
...
void turnBend(byte direction){
  ...
  while (direction == 0){
    setMotorSpeed(-80,80);
    delay(200);
    if ((analogRead(A3) < sensorThresholds[3]) && (analogRead(A4) < sensorThresholds[4])){
      return;
    }
  }
...
void convertSensorsToBinary(int sensors[], int sen[]) {
  for (int i = 0; i < 8; i++) {
    sen[i] = (sensors[i] < sensorThresholds[i]) ? 1 : 0;
  }
}
...
bool allSensorsDetectBlack(int sensors[]) {
  for (int i = 0; i < 8; i++) {
    if (sensors[i] < sensorThresholds[i]) {
...
void loop() {
  ...
  // if (analogRead(leftIR)< sensorThresholds[0]){   
  //    turnBend(0); // left bend
  //  }

  // if (analogRead(rightIR)< sensorThresholds[0]){
